...
extension Bundle {
    public var _SwiftUIX_appIconImage: SwiftUIX_Hi.AppKitOrUIKitImage? {
#if os(macOS)
        ...
        {
            return SwiftUIX_Hi.AppKitOrUIKitImage(named: .bundleResource(lastIcon, in: self))
        } else if let iconFile = infoDictionary?["CFBundleIconFile"] as? String {
            return SwiftUIX_Hi.AppKitOrUIKitImage(named: .bundleResource(iconFile, in: self))
        } else {
            return nil
        }
    }
